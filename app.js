...
const supabaseUrl = 'https://egmshprlqefhngifurnq.supabase.co';
const supabaseKey = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImVnbXNocHJscWVmaG5naWZ1cm5xIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NDk5MTkzODEsImV4cCI6MjA2NTQ5NTM4MX0.qHnXIJilElEcCnG9XIOBLRjaJeCmB05mGSWnxgp8dGM';
...
async function cargarDatos() {
  try {
    ...
    if (!productosError) productos = productosData;
    ...
    if (!sorteosError) sorteos = sorteosData;
...
function renderResumen() {
  ...
  // Calcular KPIs de sorteos
  const valorTotalSorteos = sorteos.reduce((sum, s) => sum + s.valor, 0);
  const gananciaTotalSorteos = sorteos.reduce((sum, s) => sum + s.ganancia, 0);
  const gananciaNetaSorteos = gananciaTotalSorteos - valorTotalSorteos;

  document.getElementById("resumenTotalSorteos").textContent = `${config.currency}${formatNumber(Math.round(valorTotalSorteos))}`;
  document.getElementById("resumenGananciaSorteos").textContent = `${config.currency}${formatNumber(Math.round(gananciaTotalSorteos))}`;
  document.getElementById("gananciaNetaSorteos").textContent = `${config.currency}${formatNumber(Math.round(gananciaNetaSorteos))}`;
...
// Funciones CRUD actualizadas para Supabase
...
async function agregarSorteo() {
  ...
  const nuevoSorteo = {
    ...
    valor_numero: valorNumero.value ? parseFloat(valorNumero.value) : null,
...
async function eliminarProducto(index) {
  ...
  const productoId = productos[index].id;
...
async function eliminarSorteo(index) {
  ...
  const sorteoId = sorteos[index].id;
    ...
    reader.onload = async event => {
      try {
        ...
        if (confirm(`¿Importar ${data.productos.length} productos y ${data.sorteos.length} sorteos?`)) {
          ...
          // Insertar sorteos
          const { error: sorteosError } = await supabase
            .from('sorteos')
            .insert(data.sorteos);
